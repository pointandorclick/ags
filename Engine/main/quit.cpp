...
QuitReason quit_check_for_error_state(const char *qmsg, String &errmsg, String &alertis)
{
    ...
    else if (qmsg[0]=='!')
    {
        ...
        else if (qmsg[0] == '?')
        {
            ...
            alertis = "Sierra Quest has stopped because the game asked it to. "
                "Please send the details below to support@pointandclick.studio.\n\n";
        }
        else
        {
            qreason = kQuit_GameError;
            // Sierra Quest: player-facing wording
            alertis.Format("Sorry, Sierra Quest has hit a problem and needs to close.\n"
    ...
    {
        // Sierra Quest: player-facing wording
        alertis.Format("Sorry, Sierra Quest has hit an unexpected problem and needs to close.\n"
            "Your saved games are safe. If this keeps happening, please send the "
            "details below to support@pointandclick.studio along with what you "
            "were doing at the time.\n"
            "(Build %s)\n"
